...
function elasticRods(bendModulus = 1, twistModulus = 1, totalTwist = pi)
    ...
    loop  = Observable(Point3.(eachcol([verts verts[:, 1]])))
    bases = Observable(Point3.(eachcol(curveData.midpointsR)))
    vecs  = Observable(Point3.(eachcol(curveData.u[:, 1:end-1])))

    fig = Figure(size=(1000, 1000))
    ax = LScene(fig)
    lines!(loop, linewidth=30)
    arrows3d!(bases, vecs, color=:red, lengthscale=0.1)
...
function propagateBishopFrame(curveData, startFrame)
    ...
    for i = 2 : curveData.nv
        tᵢ   = curveData.tangentsR[:,i]
        tᵢ₋₁ = curveData.tangentsL[:,i]
        θ = acos( clamp( dot( tᵢ, tᵢ₋₁ ), -1, 1 ) )
        v = cross( tᵢ, tᵢ₋₁ )
        parallelTransport[i] = AngleAxis( θ, v[1], v[2], v[3] )
    end
